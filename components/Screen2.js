...
import { View, Text, StyleSheet, TextInput, TouchableOpacity, Image, ScrollView, Modal, clearImage } from 'react-native';
    ...
    const pickImage = async () => {
      ...
      if (result.canceled === false) {
        if (result.assets && result.assets.length > 0) {
          ...
          setImageName(selectedImage.fileName || selectedImage.uri.split('/').pop());
        }
      }
    };

    // Define the clearImage function to reset the image state
const clearImage = () => {
  setImage(null);
  setImageName('');
};
      ...
      <View style={styles.timeContainer}>
        <View style={styles.timeInputBox}>
          ...
          <TouchableOpacity onPress={showEntryTimePicker}>
            <Text style={styles.input3}>{entryTime || 'Select Time'}</Text>
          </TouchableOpacity>
          <DateTimePickerModal
        ...
        <View style={styles.timeInputBox}>
          ...
          <TouchableOpacity onPress={showExitTimePicker}>
            <Text style={styles.input4}>{exitTime || 'Select Time'}</Text>
          </TouchableOpacity>
          <DateTimePickerModal
  ...
  <Text style={styles.label}>
    Image<Text style={{ color: '#BCBCBC' }}>(optional)</Text>
  </Text>
  <TouchableOpacity style={styles.optionalButton} onPress={pickImage}>
      ...
      {image ? (
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1, justifyContent: 'space-between' }}>
            <Text numberOfLines={1} style={{ flex: 1 }}>{imageName}</Text>
